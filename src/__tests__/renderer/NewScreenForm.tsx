...
function NewScreenForm( {addScreenToOrder, currentScreen, nextScreenNumber, configImages, editMode, clearOrder}) {
    ...
    const handleSubmit = (event) => {
        ...
        if (!formState.height || !formState.heightFraction || !formState.width || !formState.widthFraction || !formState.frameColor || !formState.frameType || !formState.splineType || !formState.screenType || !formState.config || !formState.pullTab) {
            // Set orderIncomplete to true if a field is not filled out
            setOrderIncomplete(true);
        } else {
            // If all fields are filled out, add the screen to the order,
            // attach a screenNumber to the screen being added to the order
